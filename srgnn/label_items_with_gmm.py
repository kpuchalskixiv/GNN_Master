...
def main(flags_str=""):
    ...
    with open(
        f"../datasets/{opt.dataset}/item_labels_{gm.n_components}_{parser_opt.init_params}_{opt.hiddenSize}_{parser_opt.run_id.split('-')[-1]}.txt",
        "wb",
    ) as f:
        pickle.dump(item_labels, f)
    with open(
        f"../datasets/{opt.dataset}/cluster_centers_{gm.n_components}_{parser_opt.init_params}_{opt.hiddenSize}_{parser_opt.run_id.split('-')[-1]}.txt",
        "wb",
    ) as f:
